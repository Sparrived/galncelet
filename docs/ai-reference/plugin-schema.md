...
```ts
// src/addons/registry.ts
function registerPlugin(def: PluginDef): void;  // Register at module load time
function getPlugin(id: string): PluginDef | undefined;
function getAllPlugins(): PluginDef[];
```

## WidgetContext (app-level)

```ts
// src/lib/context.tsx
interface WidgetContext {
  refresh: () => Promise<void>;           // Trigger full data refresh
  showResult: (msg: string) => void;      // Show transient success message
  showError: (msg: string) => void;       // Show transient error message
  onStatusChange: (status: string | null) => void; // Update window subtitle
}
function useWidget(): WidgetContext;
```

## WidgetContextValue (shell-level)

```ts
// src/widgets/WidgetContext.ts
const HEADER_H = 36;  // Widget header height (logical px)
...
Auto-resizes the Tauri window to fit content. Uses `ResizeObserver` + `MutationObserver`. Adds `HEADER_H` (36px) to measured content height. Has 4px deadband to avoid jitter.
